...
void main() {
  ...
  test('CREATE INDEX USING IVF-FLAT and query nearest neighbors', () async {
    ...
    // 3. Create IVF-FLAT index via SQL
    final resIndex = await interpreter.executeScript('CREATE INDEX idx_emb ON items(emb) USING ivf_flat;');
    ...
    final indexFile = File('$dbPath/idx_emb.ivf_flat');
    expect(indexFile.existsSync(), true);

    // 5. Run EXPLAIN to check if the planner selects IvfFlatScanNode
    ...
    print('EXPLAIN Plan:\n${resExplain.toString()}');
    expect(resExplain.toString().contains('IvfFlatScanNode'), true);
